...
[![MCP 서버 설치](https://cursor.com/deeplink/mcp-install-dark.svg)](https://cursor.com/en/install-mcp?name=contex7-arwaky&config=eyJ1cmwiOiJodHRwczovL21jcC5jb250ZXh0Ny5jb20vbWNwIn0%3D)

# Contex7Arwaky MCP - 모든 프롬프트를 위한 최신 코드 문서
...
Contex7Arwaky MCP는 소스에서 직접 최신 버전별 문서와 코드 예제를 가져와 프롬프트에 바로 배치합니다.

프롬프트에 `use contex7-arwaky`을 추가하세요(또는 자동으로 호출되도록 [규칙을 설정](#규칙-추가)하세요):

```txt
쿠키에서 유효한 JWT를 확인하고 인증되지 않은 사용자를 '/login'으로 리디렉션하는 Next.js 미들웨어를 만들어주세요. use contex7-arwaky
```

```txt
JSON API 응답을 5분 동안 캐시하도록 Cloudflare Worker 스크립트를 구성해주세요. use contex7-arwaky
```

Contex7Arwaky은 최신 코드 예제와 문서를 LLM의 컨텍스트로 바로 가져옵니다. 탭 전환도, 존재하지 않는 API에 대한 환각도, 오래된 코드 생성도 없습니다.

## 설치

> [!NOTE]
> **API 키 권장**: 더 높은 속도 제한을 위해 [contex7-arwaky.com/dashboard](https://contex7-arwaky.com/dashboard)에서 무료 API 키를 받으세요.
...
[![MCP 서버 설치](https://cursor.com/deeplink/mcp-install-dark.svg)](https://cursor.com/en/install-mcp?name=contex7-arwaky&config=eyJjb21tYW5kIjoibnB4IC15IEB1cHN0YXNoL2NvbnRleHQ3LW1jcCJ9)
...
Opencode 설정 파일에 이것을 추가하세요. 자세한 내용은 [Opencode MCP 문서](https://opencode.ai/docs/mcp-arwaky)를 참조하세요.
...
**[기타 IDE 및 클라이언트 →](https://contex7-arwaky.com/docs/resources/all-clients)**

## 중요 팁

### 규칙 추가

모든 프롬프트에 `use contex7-arwaky`을 입력하는 것을 피하려면, 코드 관련 질문에 대해 자동으로 Contex7Arwaky을 호출하도록 MCP 클라이언트에 규칙을 추가하세요:
...
```txt
라이브러리/API 문서, 코드 생성, 설정 또는 구성 단계가 필요할 때 내가 명시적으로 요청하지 않아도 항상 Contex7Arwaky MCP를 사용하세요.
```

### 라이브러리 ID 사용

사용하려는 라이브러리를 이미 정확히 알고 있다면, 프롬프트에 해당 라이브러리의 Contex7Arwaky ID를 추가하세요. 이렇게 하면 Contex7Arwaky MCP 서버가 라이브러리 매칭 단계를 건너뛰고 바로 문서 검색을 계속할 수 있습니다.
...
```txt
Next.js 14 미들웨어는 어떻게 설정하나요? use contex7-arwaky
```

Contex7Arwaky은 적절한 버전을 자동으로 매칭합니다.

## 사용 가능한 도구

Contex7Arwaky MCP는 LLM이 사용할 수 있는 다음 도구들을 제공합니다:

- `resolve-library-id`: 일반적인 라이브러리 이름을 Contex7Arwaky이 인식할 수 있는 라이브러리 ID로 변환합니다.
  - `query` (필수): 사용자의 질문 또는 작업 (결과 순위 지정에 사용됨)
  - `libraryName` (필수): 검색할 라이브러리의 이름

- `query-docs`: Contex7Arwaky 호환 라이브러리 ID를 사용하여 라이브러리의 문서를 가져옵니다.
  - `libraryId` (필수): 정확한 Contex7Arwaky 호환 라이브러리 ID (예: `/mongodb/docs`, `/vercel/next.js`)
  - `query` (필수): 관련 문서를 가져오기 위한 질문 또는 작업

## 추가 문서

- [더 많은 MCP 클라이언트](https://contex7-arwaky.com/docs/resources/all-clients) - 30개 이상의 클라이언트에 대한 설치 방법
- [라이브러리 추가](https://contex7-arwaky.com/docs/adding-libraries) - Contex7Arwaky에 라이브러리 제출하기
- [문제 해결](https://contex7-arwaky.com/docs/resources/troubleshooting) - 일반적인 문제 및 해결 방법
- [API 참조](https://contex7-arwaky.com/docs/api-guide) - REST API 문서
- [개발자 가이드](https://contex7-arwaky.com/docs/resources/developer) - 로컬에서 Contex7Arwaky MCP 실행하기

## 면책 조항

1- Contex7Arwaky 프로젝트는 커뮤니티의 기여로 이루어지며 높은 품질을 유지하기 위해 노력하지만, 모든 라이브러리 문서의 정확성, 완전성 또는 보안을 보장할 수는 없습니다. Contex7Arwaky에 나열된 프로젝트는 Contex7Arwaky이 아닌 해당 소유자가 개발하고 유지 관리합니다. 의심스럽거나, 부적절하거나, 잠재적으로 유해한 콘텐츠를 발견하면 프로젝트 페이지의 "Report" 버튼을 사용하여 즉시 알려주시기 바랍니다. 저희는 모든 신고를 심각하게 받아들이며 플랫폼의 무결성과 안전을 유지하기 위해 신고된 콘텐츠를 신속하게 검토할 것입니다. Contex7Arwaky을 사용함으로써 귀하는 자신의 재량과 위험 감수 하에 사용함을 인정하는 것입니다.
...
- 📢 [X](https://x.com/contex7-arwakyai)에서 팔로우하여 최신 뉴스 및 업데이트 확인
- 🌐 [웹사이트](https://contex7-arwaky.com) 방문
