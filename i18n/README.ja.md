...
[Smithery](https://smithery.ai/server/@upstash/contex7-arwaky-mcp) 経由で任意のクライアントに Contex7Arwaky MCP サーバーを自動的にインストールするには：

```bash
npx -y @smithery/cli@latest install @upstash/contex7-arwaky-mcp --client <CLIENT_NAME> --key <YOUR_SMITHERY_KEY>
```

Smithery キーは [Smithery.ai Web ページ](https://smithery.ai/server/@upstash/contex7-arwaky-mcp) で確認できます。
